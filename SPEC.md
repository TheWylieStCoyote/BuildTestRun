...
When a user runs `btr build`, `btr test`, `btr run`, or `btr exec <name>`, the CLI should:

1. Find the hidden config file in the current directory or one of its parents.
2. Merge any parent `.btr.toml` files with the nearest child overrides.
...
`btr validate` should parse and validate the config without executing anything.
`btr validate --strict` should fail on missing conventional commands, missing tools, missing env files, and placeholder `run` commands.
`btr init` should create a starter `.btr.toml` in the current directory, with templates for common ecosystems.
...
`btr init --interactive` should prompt for project name, root, and template.
`btr init --interactive` may also prompt for template-specific optional commands and safe structured-only mode.
`btr init --detect` should infer a starter template from common marker files like `Cargo.toml`, `package.json`, `pyproject.toml`, and `CMakeLists.txt`.
`btr init --import` should generate a starter config from common project files like `Cargo.toml`, `package.json`, `pyproject.toml`, `Makefile`, and `justfile`.
`btr init --detect --interactive` should use the detected template as the default prompt.
`btr init --print` should render the starter config to stdout without writing `.btr.toml`.
`btr init --list-templates` and `btr templates` should list starter templates with descriptions.
`btr init --template-file <path>` should render a custom starter template from a file or a directory containing a template file.
Rendered init templates should be validated before writing.
`btr list` should print available command names and optional descriptions.
`btr show <command>` should print the fully resolved command, cwd, env, and source provenance from base config, child config, profile, and platform override layers.
`btr show --tree <command>` should print the command inheritance and pipeline tree for the selected command.
`btr explain <command>` should print the command type, resolution details, and the same source provenance.
`btr which` should show the resolved config path, project root, config chain, and selected profile.
`btr schema` should print a JSON Schema document for `.btr.toml` so editors can provide validation and completion.
JSON output should use a stable envelope with `status`, `command`, and command-specific fields.
`btr doctor` should report missing commands and PATH issues, and suggest fixes for missing tools and env files.
`btr doctor --fix` should create any missing configured env files it can safely create.
Execution commands should print short end-of-run summaries with the target, status, count, and duration.
`btr show --source` should print an explicit provenance trace for the resolved command.
`btr doctor --strict` should exit non-zero when warnings exist.
`btr show <name>` should display the resolved command, cwd, timeout, and description.
`btr --workspace <path>` should resolve the project starting from the given directory.
`btr parallel <name>...` should run multiple named commands concurrently.
`btr workspace --tag <tag>` should filter discovered projects by `[project].tags`, and multiple tags should require all listed tags to be present.
...
Set `BTR_PROFILE=<name>` to apply `[profiles.<name>]` overlays.
`--profile <name>` should select a profile explicitly and override `BTR_PROFILE`.
...
`btr workspace --list` should list discovered projects, `btr workspace --name <project> --list` should filter by project name, `btr workspace --changed-only` should filter to changed projects, `btr workspace --changed-only --since <ref>` should use the given git base, and `btr workspace --name <project> <name>` should run a named command in each matching discovered project.
`btr workspace --jobs <n>` should run discovered projects concurrently.
`btr workspace --fail-fast` should stop scheduling new workspace jobs after the first failure.
`btr workspace --keep-going` should continue scheduling remaining workspace jobs after failures.
`btr workspace --order name` should order workspace execution by project name instead of discovery path.
...
`btr watch <command>` should rerun the selected command when files change.
`btr watch workspace ...` should rerun workspace commands on file changes.
`btr watch --once` should run a single cycle and exit, which is useful for tests and scripting.
...
- `commands.build`: Command executed by `btr build`.
- `commands.test`: Command executed by `btr test`.
- `commands.run`: Command executed by `btr run`.
- `commands.<name>`: Additional named commands executed by `btr exec <name>`.
- `commands.<name>.steps`: Sequential named steps for pipeline commands.
- `profiles.<name>`: Optional environment and command overlays activated by `BTR_PROFILE`.
...
- `btr build` succeeds with a valid config
- `btr test` fails cleanly when the configured command fails
- `btr run` reports missing config clearly
- `btr exec <name>` runs named commands
...
1. `btr build`, `btr test`, and `btr run` work for any project with a valid config.
